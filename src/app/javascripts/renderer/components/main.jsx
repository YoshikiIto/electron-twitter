...
class Tweet extends React.Component {
    render() {
        ...
        return ( < li className = 'list.group-item' > <img src={status.user.profile_image_url_https} className='img-rounded media-object pull-left' width='32' height='32'/> < div className = "media-body" > <strong className="user-name">{status.user.name}</strong> < span className = "user-screen_name" > @ {status.user.screen_name} < /span>
        ...
        }
        {
            isRetweet
                ? <span className="icon icon-retweet">
                        Retweeted by {this.props.tweet.user.name}</span>
                : null
        } < /div>
			</li >);
    }
...
module.exports = class MainContent extends React.Component {
    ...
    componentDidMount() {
        T.get('statuses/home_timeline').catch(error => {
            console.log(error);
        }).then((result) => {
